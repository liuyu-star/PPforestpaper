...
\if1\blind
{
  ...
  \bigskip
  \begin{center}
    {\LARGE\bf A Projection Pursuit Forest Algorithm for Supervised Classification}
\end{center}
  \medskip
} \fi
...
Trees that use linear combinations of predictors in a split are known in the literature as oblique trees~\citep{kim2001,brodley,tan2005mml,truong2009fast,lee2013pptree}. All these algorithms use different approaches for finding linear combinations of predictors upon which to make a split. Some of the methods used for selecting the linear combination include random coefficient generation, linear discriminant analysis, and linear support vector machines. Theoretically, these could also be used as a base underlying PPF.
...
This paper is organized as follows. Section \ref{PPFsec} describes the PPF algorithm and explains the PPtree algorithm underlying PPF; diagnostics, including how to compute variable importance and the implementation details. Section \ref{perfsec} evaluates the algorithm performance on benchmark machine learning data in comparison with other tree methods. Section \ref{options} explains the diagnostics in comparison to those from RF, and discusses parameter selection. An application illustrating the power of PPF over other tree methods is provided in Section \ref{rnaapli}. Section \ref{discpp1} discusses possible extensions and future directions.
\section{Projection pursuit random forest}\label{PPFsec}


This section provides the definition of PPF for classification, background on the projection pursuit tree, an explanation of the PPF algorithm, implementation details, and a description of how the diagnostics are defined and computed. 

\subsection{Definition}\label{PPFdef}

This description follows closely the original random forest notation in \cite{breiman2001random}. The next two sections use this notation for the PPtree and PPF algorithms.
Let  $d_n =\{(\mathbf{x_i},y_i)\}_{i=1}^n$ be the training data set where $\mathbf{x_i}$ is a $p$-dimensional column vector of explanatory variables with $i=1,\ldots n$, $y_i\in \mathscr{G}$ (where $\mathscr{G}$ is a set of unique classes labeled and numbered $g =1,2,\ldots, G$). $G$ is the number of classes and  $n_g = \sum_{i=1}^n I(y_i=g)$ is the number of observations in class $g$.

A projection pursuit classification random forest can be defined as a collection of randomized projection pursuit classification trees $\{ h(\mathbf{x}, \Theta_k), k = 1, \cdots, B$\}, where $\mathbf{x} \in \Re^p$ and $\{\Theta_k\}$ are i.i.d random vectors, incorporating two sources of randomness in the tree: random variable selection and random bootstrap samples. Thus, this $\Theta_k$ contains information about which variables were selected in each partition and which cases were selected in the bootstrap sample.

For each randomized tree, $h(\mathbf{x}, \Theta_k)$, a unique vote is collected based on the most popular class for the selected predictor variables.  Equation \ref{rfesti} defines the PPF estimator based on combining the trees.

\begin{eqnarray}\label{rfesti}
f(\mathbf{x})&=& \operatorname*{arg\,max}_{g\in \mathscr{G}} \{E_{\Theta}(I(h(\mathbf{x}, \Theta)=g)\}\\ \nonumber
&=& \operatorname*{arg\,max}_{g\in \mathscr{G}} P_{\Theta}(h(\mathbf{x}, \Theta)=g)
\end{eqnarray}

\noindent $E_{\Theta}$ is the expectation w.r.t. $\Theta$.
In practice, the PPF estimator is evaluated by generating $B$ random trees and takes the average of the individual outcomes. This procedure is justified in a similar way to the original random forest defined by \cite{breiman2001random}, and is based on the Law of the Large Numbers~\citep{athreya2006measure}.

Equation \ref{predfor} describes the prediction of a new observation $\mathbf{x}_0$,

\begin{equation}
\hat f(\mathbf{x}_0)= \operatorname*{arg\,max}_{g\in \mathscr{G}} \sum_{k=1}^B I ( h (\mathbf{x}_0, \Theta_{k} )= g).
\label{predfor}
\end{equation}



\subsection{Background on the projection pursuit tree}
\label{PPT}
...
@
 In this subsection, two key components to understand the PPF algorithm are described: projection pursuit indexes and how they are used to define projection pursuit classification trees (PPtree).
 
The projection pursuit algorithm searches for low dimensional projections, by optimizing a function that measures some aspect of interest. For PPtree, this is class separation. \cite{friedman1973projection} coined the term ``projection pursuit'', but the ideas existed earlier than this, and are discussed in~\cite{kruskal1969toward}. Indexes for finding projections that separate classes were developed in~\cite{lee2005projection} (LDA) and~\cite{lee2010projection} (PDA). These form the basis of PPtree, and are explained below.
...
@
\caption{Examining the behavior of the LDA index on two different 2D data sets, using a Huber plot (left) displaying index values for all 1D projections. A scatterplot in the center shows the data. The solid line indicates index values, on a standardized scale. The dashed circle corresponds to the median index value over all 1D projections. The dotted line indicates the maximum index value, and the corresponding data projection is shown on the right, as a density plot. \label{huberpl}}
\end{figure}

Figure \ref{huberpl} examines the behavior of the LDA index on two 2D simulated data sets, using a Huber plot~\citep{huber1990data}, that is also available in the \texttt{PPtreeViz} package~\citep{PPtreeViz}. The projection pursuit index values are calculated for all 1D projections of the 2D data, by parametrizing the projections using an angle $\theta$. The projection vectors are generated using ($cos~\theta$, $sin~\theta$), for $\theta$ = $1^{\circ}$, . . . , $180^{\circ}$. The index value is computed by projecting the data  accordingly, and displayed as a line (solid) winding around the data scatterplot. The circle (dashed line) is a guideline, and corresponds to the median of all index values. The projection vector producing the maximum index value is indicated as a dotted line, and the corresponding projected data is shown in the density plot (right). The first data set (top row) was simulated without separation between the classes, and this is reflected by a fairly uniform index function, that is close to the median for all projections. The best separation simply reflects sampling variability that one group (green) is slightly shifted farther afield than the other two classes. The second data set (bottom row) has separated classes visible in a 1D projection, and each class has strong correlation between the two variables. The LDA index is smooth, varying substantially with projection, with a flattish peak at the optimal projection. The LDA index easily finds the separation between these classes.
...
The PPtree algorithm uses a multi-step approach to fit a multi-class model by finding linear combinations to split on. Figure \ref{bounds} compares the boundaries that would result from a classification tree (fitted using the R package, `rpart`~\citep{rpartpkg}) and the PPtree algorithm. Figure \ref{diagpp1} illustrates the PPtree algorithm for three classes, and the algorithm steps are detailed below.


\begin{enumerate}
\item Optimize a projection pursuit index to find an optimal one-dimensional projection, $\pmb{a_1}^*$, for separating all classes in the current data yielding projected data $z_i = {\pmb{a_1}^*}^T\mathbf{x_i}$ for all $i$.
\item On the projected data, redefine the problem into a two class problem using the distances among the means of classes, and assign a new label, either $g_1^*$ or $g_2^*$ to each observation, generating a new class variable $y_i^*$. Note that this requires joining two or more groups into one, so that the new groups $g_1^*$ and $g_2^*$ possibly contain more than one of the original classes.

\item Find an optimal one-dimensional projection $\pmb{a_1}^{**}$, using $\{(\mathbf{x_i},y_i^*)\}_{i=1}^n$ to separate the two class problem $g_1^*$ and $g_2^*$. The best separation of $g_1^*$ and $g_2^*$ is determined in this step providing the decision rule for the node,

\begin{quote}
if $\pmb{a_1}^{**T}\bar{\mathbf{x}}_{g_1^*}< c$ then assign $g_1^*$ to the left node else assign $g_2^*$ to the right node,
\end{quote}
\noindent where $\bar{\mathbf{x}}_{g_1^*}$ is the mean vector of $g_1^*$. Split value on projected data, is computed as $c=0.5*m_1+ 0.5*m_2$ (default), where $m_1$ and $m_2$ are group means on projected data. There are eight possible rules for computing $c$ (see \cite{PPtreeViz} for more details).
...
\subsection{PPF Algorithm }

A projection pursuit classification random forest can be defined as a collection of randomized projection pursuit classification trees $\{ h(\mathbf{x}, \Theta_k), k = 1, \cdots, B$\} defined in subsection \ref{PPFdef}. The PPtree algorithm underlying PPF was mentioned before. In this subsection PPF algorithm is described.

\begin{enumerate}

\item
$B$ stratified bootstrap samples from $d_n$ are taken. For each group, $n_g$ cases are re-sampled from $d_{n_g}=\{(\mathbf{x}_i, y_i)| y_i = g, i=1, \cdots, n\}$ (training data set for group $g$) independently and uniformly. We denote this $k$th bootstrap sample from $g$th group as $b_{k_g}$. The $k$th bootstrap sample $b_k=\{b_{k_g}:g=1, \cdots, G\}$.

\item Use a bootstrap sample $b_k$ to grow a randomized PPtree $(h(\mathbf{x}, \Theta_k))$ (Note that the depth of the PPtree is at most $G-1$, where $G$ is the number of classes).

\begin{enumerate}
\item Start with all the cases in $b_k$ in the root node.
\item Select $m$ predictor variables from the data in the node where $m<<p$. 
\item Find the optimal one-dimensional projection $\pmb{a_1}^*$ to separate all the classes in $b_k$.
...
\item Find the optimal one-dimensional projection, $\pmb{a_1}^{**}$, using $b_k$ with selected $m$ variables and the relabeled response, $y^*$, to separate $g_1^*$ and $g_2^*$.
The linear combination is computed by optimizing a projection pursuit index to get a projection of the variables that best separates the classes using the $m$ randomly selected variables. Two index options are available LDA or PDA.
\item Compute the decision boundary $c$ (see PPtree algorithm above).
\item Keep $\pmb{a_1}^{**}$ and $c$.
\item Separate the data into two groups using the new labels $g_1^*$ and $g_2^*$ and assign $g_1^*$ to the left node and $g_2^*$  to the right node.
\item Repeat from (b) to (h) if $g_1^*$ or $g_2^*$ have more than two original classes.
\end{enumerate}
\item Repeat 2 for $k = 1,\ldots, B$.
\item The output is the ensemble of randomized PPtrees, $\{h(\mathbf{x}, \Theta_k)\}_{k=1}^B$.
\end{enumerate}
...
@
\vspace{-3cm}
\caption{Computational performance for different sample size ($n$), number of variables ($p$), number of groups ($G$), number of trees ($B$) and proportion of variables considered for each node. Computational speed suffers most when the proportion of variables is high.\label{ratiotim}}
\end{figure}
...
PPF calculates variable importance in three ways: permuted importance using accuracy,  and two importance measures based on projection coefficients on standardized variables. 

The permuted variable importance is comparable to the measure defined in the classical random forest algorithm. It is computed using the oob cases for the tree $k\;\;(O_k)$ for each $j$ predictor variable.  Then the
permuted importance of the variable $j$ in the tree $k$ can be defined as:
...
For the other two importance measures, the coefficients of each projection are examined. The magnitude of these values indicates importance if the variables have been standardized. The variable importance for a single PPtree is computed by a weighted sum of the absolute values of the coefficients across node, then the weights take the number of classes in each node into account($G_s$)~\citep{lee2013pptree} .
The importance of the variable $j$ in the PPtree $k$ can be defined as $\sum_{s= 1}^{S}\frac{|a^{**}_{sjk}|}{G_s }$ where $a^{**}_{sjk}$ is the projected coefficient for node $s$ and variable $j$ in the $k^{th}$ PPtree, and $S$ is the total number of node partitions in the $k^{th}$ tree.
...
\]
Alternatively, a global importance measure is defined for the forest as a weighted mean of the absolute value of the projection coefficients across all nodes in every tree.
\[VI^{(3)}_j=\frac{1}{B}\frac{1}{G-1}\sum_{k=1}^B\sum_{s = 1}^{S} (1-e_k) I_{sjk}|a^{**}_{sjk}|
\]

\noindent where $e_k$ is the oob error rate of tree $k$, $I_{sjk}$ is the projection pursuit index value of node $s$.
...
Given a training data set $d_n$, $B$ bootstrap samples from $d_n$ are taken. For each bootstrap sample ($k= 1, 2, \ldots, B$), a \verb# PPtree# classifier $h(\mathbf{x}, \Theta_k)$ is constructed, and a majority vote is used to get the PPF predictor.
The oob cases are used to get the error rate estimates. For each $(\mathbf{x_i}, y_i)$ in $d_n$, the votes are aggregated only for the classifiers $h(\mathbf{x}, \Theta_k)$ that do not contain $(\mathbf{x_i}, y_i)$. Hence, PPF is called the out-of-bag classifier, and the error rate for this classifier (oob error rate) is the estimate of the generalized error. The oob error rate is a measure for each model that is combined in the ensemble and is used to provide the overall error of the ensemble.
...
This section presents a benchmark data study to examine the predictive performance of PPF in comparison to other methods. 
...
@
\caption{Benchmark data results shown graphically. Color indicates data set, with lines linking the error rates for the data using different classifiers, on both the training and test sets. PPF performs consistently well across most of the data sets. \label{parallel}}
\end{figure}
...
The primary parameters for PPF are mostly the same as those for RF: number of trees, and number of variables used in each node partition, with the addition of $\lambda$ when the PDA index is used.

Figure \ref{parameters} (left) shows the effect of proportion of variables for the benchmark data comparison. The average error over 200 training/test splits is shown. Generally, the error is lower when the more variables are used. For leukemia and NCI60, the error increases when more variables are included. These two examples are both small $n$, big $p$ data, which suggests that increasing the number of variables will be not the best strategy to reduce error. Section \ref{rnaapli} illustrates this.

Figure \ref{parameters} (right) compares  PPF and RF on the number of trees needed to optimize the oob error for the lymphoma data. Both need around 100 trees to produce best performance.
...
We applied the proposed method to analyze an RNA-seq gene expression data set. The data are high-dimensional, with the number of observations, $n$ is very small relative to the number of features, $p$. Traditional classification methods like linear discriminant analysis or logistic regression cannot be applied directly in this context. PPforest with the PDA index can be applied to this type of data. 
...
We set up the classification problem as in \cite{tan2014classification}, where the response variable is the genotype (four classes, B73, Mo17, B73xMo17 and Mo17xB7) and the feature variables are the expression of the 1078 genes. The goal is to identify important genes to distinguish among genotypes. The subset is formed with genes that are already found to be relevant, for distinguishing between inbred genotypes and its reciprocal hybrids, because choosing genes with smallest $p$-values can be misleading \citep{cook2007exploring}. To compare the predictive performance among different methods we will use leave-one-out cross validation (LOOCV) since we have a limited amount of data (16 observations).
...
\begin{figure}[!ht]
\centering
<<rnaseq, echo=FALSE, fig.height=5, fig.width=8, warning=FALSE, message=FALSE>>=
...
sca_rna1 <- dd_sc  %>%
  dplyr::select(imp_genes10,Type ) %>%
  ggplot(aes(x = GRMZM2G046776, y = GRMZM2G125969, color = Type)) + geom_point(size = I(3), alpha = 1/2) + scale_colour_brewer(type = "qual", palette = "Dark2") + theme(legend.position = "none", aspect.ratio = 1) +scale_x_continuous( limits = c(-2,2.7))+ scale_y_continuous( limits = c(-2,2.7))                                                                                                                   

sca_rna2 <- dd_sc  %>%
  dplyr::select(imp_genes10,Type ) %>%
  ggplot(aes(x = GRMZM2G093708, y = GRMZM2G014695, color = Type)) + geom_point(size = I(3), alpha = 1/2) + scale_colour_brewer(type = "qual", palette = "Dark2") + theme(legend.position = "none", aspect.ratio = 1) + scale_x_continuous( limits = c(-2,2.7)) + scale_y_continuous( limits = c(-2,2.7))

sca_rna3 <- dd_sc  %>%
  dplyr::select(imp_genes10,Type ) %>%
  ggplot(aes(x = GRMZM2G125969, y = GRMZM2G075892, color=Type))+geom_point(size = I(3), alpha = 1/2) +scale_colour_brewer(type = "qual", palette = "Dark2")+ theme(legend.position="none", aspect.ratio = 1) + scale_x_continuous( limits = c(-2,2.7)) + scale_y_continuous( limits = c(-2,2.7))

ppfimpo1_rna <- ggplot( globalimpo_rna[1:50,], aes(x = mean, y = variable)) + geom_point() + theme(aspect.ratio = 1, axis.text.y = element_blank()) + labs(x = "Global aver. importance (VI2)", y="")
...
Table \ref{res_RNA} presents the mean of the training and test error rates from leave-one-out cross validation comparing CART, PPF, PPtree and RF. For PPF, two cases are presented based on using a different proportion of variables for each node partition: PPF.4 use $40\%$ of the variables while  PPF.03 use $3\%$ of them. PPF.03, with 32 variables in each node partition, has a similar number of variables to the default for RF ($\sqrt{p}$). PPF with $40\%$ of variables, and PPtree, produce the same in test error ($6.2\%$). The performance of PPF is better when a smaller proportion of variables ($3\%$) is used (resulting in $0\%$ of test error). Even though RF has a similar number of variables used, with a test error of $25\%$ it performs worse than PPF.03.

Figure \ref{rnacomp} shows the most important genes analyzed based on global average importance measure for the best model (PPF.03). The importance measure is clearly bigger for one gene (GRMZM2G046776) and there is a breaking point in the importance measure for the first seven genes. The parallel coordinate plot shows the expression values for the top 10 variables, ordered from left to right. In the left scatterplot, of first two important variables, Mo17xB73 can be clearly separated from the other genotypes. The other scatterplots show various separations between the groups. 
...
The benchmark data study showed that PPF's predictive performance is as good, or better than, CART and PPtree, and can outperform RF for some problems. PPF performs better than RF when the classes are separated by a linear combination of variables and when the correlation between variables is high. This can be seen by comparing the variable importance diagnostics of PPF and RF, showing that different variables are combined to create the classification model. Generally, we tend to use 50\% of the variables, for PPF, but setting the proportion of variables to be sampled at each node is left to the user. Using all variables at each node would require careful tuning the $\lambda$ parameter of the PDA index to reduce the dimensionality, and may violate the assumption by construction of independent trees, as explained in \cite{breiman2001random}. 
...
This paper was written with the R packages knitr~ \citep{xie:2015}, ggplot2 \citep{hadley:2009} and dplyr \citep{dplyr}, and the files to reproduce the paper and results is available at \url{https://github.com/natydasilva/PPforestpaper}.
